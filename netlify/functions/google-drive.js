/**
 * Netlify Function - Consolidated Google Drive operations
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { operation, folderName, folderId, filename, base64Data, mimeType, clientId, clientSecret, refreshToken, parentFolderId, gridPages } = body;
    
    // For proxy-image (GET request), credentials come from query params
    const isProxyImage = operation === 'proxy-image' || (!operation && event.httpMethod === 'GET' && queryParams.fileId);
    ...
    // Handle create-grid-folder operation (creates folder and returns folderId)
      ...
      if (!folderResponse.ok) {
        ...
        return {
          ...
          body: JSON.stringify({
            error: `Failed to create folder: ${folderResponse.status} - ${errorText}`
          }),
        };
      }
    ...
    // Handle upload-single-grid operation (uploads one grid image at a time)
      ...
      const { folderId, base64Image, fileName } = body;
      ...
      try {
        ...
        if (!uploadResponse.ok) {
          ...
          return {
            ...
            body: JSON.stringify({
              success: false,
              error: `Failed to upload file: ${uploadResponse.status} - ${errorText.substring(0, 200)}`,
            }),
          };
        }
