/**
 * Netlify Function - Consolidated Etsy operations
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { operation, shopUrl, listingIds, listingId, url, apiKey } = body;

    // Use provided API key or fall back to environment variable
    const etsyApiKey = apiKey || queryParams.apiKey || process.env.ETSY_API_KEY || process.env.VITE_ETSY_API_KEY;
    
    if (!etsyApiKey && operation !== 'proxy-image' && queryParams.operation !== 'proxy-image' && !queryParams.url && !url) {
      return {
        statusCode: 500,
        headers: {
    ...
    if (operation === 'analyze' || (!operation && shopUrl)) {
      ...
      if (!shopUrl) {
        return {
          ...
          body: JSON.stringify({ error: 'Shop URL is required' }),
        };
      }
      ...
      if (!pathMatch) {
        return {
          ...
          body: JSON.stringify({ error: 'Invalid Etsy shop URL format' }),
        };
      }
    ...
    // Handle fetch-images operation
      ...
      if (!listingIds || !Array.isArray(listingIds) || listingIds.length === 0) {
        return {
          ...
          body: JSON.stringify({ error: 'Listing IDs array is required' }),
        };
      }
    ...
    // Handle listing operation (GET listing images)
    } else if (operation === 'listing' || queryParams.operation === 'listing' || (!operation && queryParams.listingId)) {
      ...
      const listingIdParam = queryParams.listingId || listingId;
      ...
      const imagesResponse = await fetch(
        `https://openapi.etsy.com/v3/application/listings/${listingIdParam}/images`,
        { headers }
      );
    ...
    } else if (operation === 'proxy-image' || queryParams.operation === 'proxy-image' || (!operation && (queryParams.url || url))) {
      if (event.httpMethod !== 'GET') {
        return {
          statusCode: 405,
          headers: {
