...
const EtsySEOOptimizer: React.FC<EtsySEOOptimizerProps> = ({ onClose }) => {
    ...
    const evaluateListing = async (listing: { title: string, description: string, tags: string[] }): Promise<SEOScore> => {
        const promptLines = [
            ...
            '=== EVALUATION CRITERIA ===',
            'TITLE:',
            '- Sharpness over Length: A 100-120 character highly-targeted title is GREAT. However, DO NOT penalize length slightly over 130 if it is packed with strong buyer intent.',
            '- Commercial Strength: REWARD titles that retain clear, high-intent product phrasing (e.g., "Junk Journal Kit" is better than the vague "Vintage Journal").',
            '- Truncation: Heavily penalize if the title ends in a cut-off or partial word.',
            '- Focus: MUST have ONE dominant buyer-intent phrase. Secondary phrases must support this identity.',
            ...
            '- Specificity: Tags must be hyper-specific to the product/theme/use-case.',
            '- Allowance: Standard commercial terms like "digital download" or "printable art" are ACCEPTABLE and should NOT be heavily penalized if used sparingly alongside highly targeted aesthetic tags.',
            '',
            'DESCRIPTION:',
