...
const EtsySEOOptimizer: React.FC<EtsySEOOptimizerProps> = ({ onClose }) => {
    ...
    const evaluateListing = async (listing: { title: string, description: string, tags: string[] }): Promise<SEOScore> => {
        const promptLines = [
            ...
            '- Quality: Must be transactional/buyer-intent phrases. Penalize generic one-word tags.',
            '- Aesthetics: Must contain at least 2 aesthetic/subculture signaling tags.',
            '',
            'DESCRIPTION:',
  ...
  "weaknesses": ["string", "string"]
    ...
    const handleOptimize = async () => {
        ...
        const promptLines = [
            ...
            'Each tag must pass this test: If a buyer types this exact phrase into Etsy search, would they realistically be looking to BUY this product?',
            'If the phrase feels decorative, abstract, or informational rather than transactional, DO NOT use it.',
            'MANDATORY FORMULA: Product + Theme + Use Case (e.g., "pink floral ephemera", "sakura journal kit", "spring junk pages").',
            'NEVER use weak combinations like "botanical junk", "asian floral craft", "printable paper set", or "digital download" alone.',
            'Avoid: vague aesthetic labels alone, generic digital terms, non-purchase phrases.',
            '',
